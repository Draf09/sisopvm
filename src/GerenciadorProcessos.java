...
public class GerenciadorProcessos {
    ...
    public GerenciadorProcessos(GerenciadorMemoria gm, Word[] memory, CPU cpu, Semaphore escSemaforo, Escalonador escalonador) {
        process_id=0;
        this.cpu = cpu;
        this.gm = gm;
        this.memory = memory;
    ...
    public PCB nextProcess (int process_id){
        PCB next = null;
        for (int i = 0; i < prontos.size(); i++) {
            if (prontos.get(i).id==process_id){
                next = prontos.get(i+1);
            }
        }
        return next;
    }
    ...
    public int criaProcesso(Word[] programa){
        ...
        PCB pcb =  new PCB(process_id, paginasAlocadas, 0, new int[10], new Word(Opcode.___, -1, -1, -1), Interrupts.INT_NONE, context);
        pcb.setNomeDoPrograma(nomeDoPrograma);
